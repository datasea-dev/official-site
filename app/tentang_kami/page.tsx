...
export default async function TentangKami() {
  ...
  // 1. Ambil Data Tim & Visi Misi secara Paralel
  const [allTim, visiMisiData] = await Promise.all([
    getTimData(),
    getVisiMisiData()
  ]);

  const ketua = allTim.find(m => m.divisi === "BPH" && (m.jabatan.toLowerCase().includes("ketua") || m.jabatan.toLowerCase().includes("president"))) || null;
  ...
  // --- DATA DIVISI DENGAN BANNER IMAGE ---
  const DIVISIONS = [
    {
      id: "bph",
      name: "BPH",
    ...
    <main className="relative min-h-screen flex flex-col text-slate-900 overflow-hidden">
      ...
      <section className="relative z-10 py-20 px-6 bg-[#1e293b] text-white">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-12 items-center">
          ...
          <div className="w-full md:w-1/3 flex flex-col items-center text-center">
            ...
            <h3 className="text-xl font-bold text-white">{ketua?.nama || "Ketua Komunitas"}</h3>
            <p className="text-blue-400 text-sm font-medium uppercase tracking-widest mb-4">President of Datasea</p>
      ...
      <section className="relative z-10 py-24 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-datasea-blue">Kenali Divisi Kami</h2>
          ...
          {DIVISIONS.map((div) => (
            ...
            >
              <div className="h-full bg-white rounded-3xl p-8 border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden flex flex-col">
                ...
                <h3 className="text-2xl font-bold text-datasea-blue mb-2 group-hover:text-blue-600 transition-colors">
                  {div.name}
                </h3>
